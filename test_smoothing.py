...
if adaptive:
    ...
    state_eq = lambda x, p: np.clip(
        x + p + np.random.normal(0, 1e-5, N_particles), 0, 1
    )
    param_eq = lambda p: np.clip(
        p + np.random.normal(0, w_param, N_particles), 0, np.inf
    )
...
particle_filter = SIR_PF(
    obs_eq, state_eq, param_eq, backprop_eq, N_particles, obs_std, thresh, M
)
